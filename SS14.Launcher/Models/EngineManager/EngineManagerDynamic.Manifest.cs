...
public sealed partial class EngineManagerDynamic
{
    ...
    private async Task UpdateBuildManifest(CancellationToken cancel)
    {
        ...
        Log.Debug("Loading Robust manifest from {manifestUrl}...", ConfigConstants.RobustBuildsManifest);
        ...
        {
            // Make sure both manifests downloaded okay, or don't blow up existing one.
            Log.Warning("Problem getting either of the build manifests, not updating any existing cached manifest so as to not lose data.");
            return;
        }

        // Start using upstream's mirrored manifest
        _cachedRobustVersionInfo = freshRobustVersionInfo;

        // Merge MV builds into the main cache file
        foreach (var kvp in freshMultiverseVersionInfo)
        {
            if (_cachedRobustVersionInfo.ContainsKey(kvp.Key))
            {
                // Generally intended that each manifest file should define its own builds.
                Log.Warning("Multiverse builds manifest redefined existing Robust Toolbox build:  {key}", kvp.Key);
                _cachedRobustVersionInfo[kvp.Key] = kvp.Value;
            } else {
                _cachedRobustVersionInfo.Add(kvp.Key, kvp.Value);
            }
        }
